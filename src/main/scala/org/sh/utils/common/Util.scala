...
//    java.util.Base64.
  ...
  //  def trycatchParNew[B](list:List[unitToT[B]]):B = { // 10000 = 10 seconds
  ...
  //    }.head
  //  }
  //1 to 100 foreach{i => println(trycatchPar(List(() => {println("1"); ???},() => {println("2");Thread.sleep(1000);2},() => {println("3");???},() => {println("4");4}, ()=>{println("5");5})))}
  //1 to 10 foreach{i => println(trycatchPar(List(() => ???,() => {Thread.sleep(100000);2},() => {???},() => {4}, ()=>{5})))}
