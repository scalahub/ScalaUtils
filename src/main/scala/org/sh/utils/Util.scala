...
object Util {
  ...
  def shaSmall(s: String): String =
    (cleanString(sha256(s)) match { // removes non-alphanumeric chars such as / and =
  ...
  def getOrElseExit[T](func: () => T) = try { func.apply } catch {
    case e: Exception =>
